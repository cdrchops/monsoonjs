        ...
        gpio.then((zones) => {
            try {
                ...
                if(action.toLowerCase() === 'off') {
                    reply({message:'turned off zone: ' + pinNum}).code(200);
                    return zones[[pinNum]].close();
                }
                reply({message:'turned on zone: ' + pinNum}).code(200);
                return zones[pinNum].open();
            } catch (err){
                if(pin.toLowerCase() === 'all') {
                    if(action.toLowerCase() === 'off') {
                        reply({message:'turned off all zones'}).code(200);
                        return zones.close();
                    }
                    reply({message:'turned on all zones'}).code(200);
                    return zones.open();
